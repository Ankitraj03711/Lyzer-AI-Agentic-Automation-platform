...
import { connectGoogleDrive } from '@/lib/helpers/socialHelpers';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const redirectUri = `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/auth/googleDrive/callback`;
    ...
    // Save to GoogleDrive table and update Social table
    await connectGoogleDrive(user._id, {
    ...
    return NextResponse.redirect(new URL('/connections?success=google', process.env.NEXT_PUBLIC_APP_URL));
  } catch (error) {
