...
if api_key:
    print(f"OPENROUTER_API_KEY loaded: {api_key[:5]}...{api_key[-4:]}")
else:
...
@app.get("/test-llm")
async def test_llm():
    """Test endpoint to verify OpenRouter/LiteLLM connection works."""
    ...
    try:
        ...
        print("DEBUG: Testing OpenRouter connection...")
        ...
        print(f"DEBUG: OpenRouter test success: {result}")
        return {"status": "ok", "response": result, "model": "openrouter/openai/gpt-4o"}
    except Exception as e:
        import traceback
        traceback.print_exc()
